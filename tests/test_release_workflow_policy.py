...
def test_adapters_tag_version_mismatch_is_rejected() -> None:
    # The package currently declares 0.3.0; a tag naming a different version
    # must fail closed rather than publish a mismatched release.
    pkg_version = re.search(
        r'(?m)^version\s*=\s*"([^"]+)"',
        (ROOT / "adapters" / "nornyx-agentic-adapters" / "pyproject.toml").read_text(encoding="utf-8"),
    ).group(1)
    assert pkg_version == "0.3.0", "test assumption drifted from the actual package version"
    assert _adapter_eligible("adapters-v0.3.1") is False
