...
> - Explain why fused, appliance-style control implementations resist governance, and how decomposing control functions behind named, versioned interfaces makes drift detectable and responsibility assignable.
...
## 2.5 Fused controls versus a decomposed control plane

Look again at the artifact set of Section 2.1 as a single implementation. Every governance function Northstar needs is present — a rule is stated, entitlements are judged, actions are sometimes blocked, records of a kind accrue — but the functions exist *fused*, welded into the agent's execution environment and its surrounding scripts with no seams between them. A fused control implementation resists governance for three specific reasons. Its functions have no names: "which component decides whether this action is permitted?" has no answer, because the deciding is smeared across a prompt, a regex, and a checkbox. Its interfaces are neither specified nor versioned: no artifact can be checked against another, and a change to any part is a change to an undocumented whole. And its authority is not separable: proposing an action, evaluating it, and recording the outcome all happen inside one execution environment maintained by whoever last touched it, so no party can be made responsible for one function without inheriting all of them.

The structural cure is <span class="ix" data-ix="functional decomposition">functional decomposition</span>: give each control function a name, a separately specified and versioned interface, and an authority that can be assigned to exactly one owner. Declaring policy, deciding a described action, enforcing the decision at the action boundary, and recording what was decided become distinct elements, and the deciding-and-recording elements form a <span class="ix" data-ix="control plane">control plane</span> architecturally distinct from the path that acts. Two consequences follow directly. Drift becomes *detectable*, because divergence can only be checked between representations that have identities and interfaces to compare across — the missing edges of Figure 2.1 exist only in a decomposed design. And responsibility becomes *assignable*, because "which component decides?" and "which component enforces?" acquire exactly one answer each, which is what lets an incident review name the function that failed rather than the team that was nearest.

Figure 2.3 puts Northstar's current state and the decomposed target side by side. The decomposition is not free — more elements, more interfaces to specify, and a planner that must now be treated as an untrusted layer rather than a trusted participant — but every requirement in Section 2.6 presupposes it, because each one needs a function that can be pointed at.
...
<figure class="nx-fig" id="fig-2-3">
  <div class="fig-body">
    ...
    <div class="layers">
      ...
      <div class="layer untrusted" data-note="Probabilistic; not a control surface">Planner and framework</div>
    </div>
  </div>
  <figcaption><b>Figure 2.3 — Fused controls versus a decomposed control plane.</b> Top: the state described in Section 2.1, where every governance function is fused into the agent's execution environment and its surrounding scripts. Bottom: the decomposition this book develops, with the planner deliberately drawn as an untrusted layer rather than a participant in governance. The teaching purpose is to show that the cure is *naming the functions and the interfaces between them*, not adding components.</figcaption>
</figure>
...
5. A team's agent platform is one deployable unit that assembles the prompt, evaluates its own safety rules, executes tool calls, and writes its own logs. Explain why drift inside such a fused implementation is undetectable from outside, and name the two properties a decomposition must give each control function before an incident review can assign responsibility for a failure.
