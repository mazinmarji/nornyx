...
*Order-dependent resolution* — first applicable wins — is the alternative, and it is a trap in a governance context, because it makes the meaning of a rule set depend on the sequence of a text file. Two documents with identical rules in different orders decide differently, which destroys the semantic-identity property that Chapter 8 needs for composition and canonicalization. XACML supports a family of combining algorithms including first-applicable and both override directions [@xacml]; the flexibility is genuinely useful in the large federated deployments XACML targets and is a liability in a system whose policies are edited by many teams.
...
Three honest qualifications. Closing the top level of a document is not the same as closing every block inside it, and a language that closes only the outer envelope still admits typos in the interior; the strength of the guarantee tracks exactly how deep the closure goes. Second, closed schemas make extension a versioned event, which is the point but also a cost: every new field is a schema revision, and a language that changes often will feel obstructive. Third, a closed schema constrains syntax, not meaning. It cannot prevent a well-formed policy from being a bad policy, and treating schema validity as policy correctness is a category error that Chapter 15's testing discipline exists to correct.
...
Four classes of test follow, and a governance suite needs all four. <span class="ix" data-ix="policy testing">Allow tests</span> assert that intended work proceeds, and they are the ones teams write first and value most, because they catch the over-restriction that makes governance unusable. **Deny tests** assert that the prohibited action is refused, with the expected diagnostic — asserting the *reason*, not merely the refusal, because a rule that denies for an accidental reason will stop denying when the accident is fixed. **Approval-required tests** assert that the third outcome appears where it should, which is the class most often missing and the one that catches a control quietly collapsing into allow. And **failure tests** assert what happens when the machinery itself misbehaves: a missing policy artifact, an unresolvable subject, a malformed instant. Chapter 15 develops this into a full conformance discipline, and Chapter 14 adds the fifth class, bypass tests, which live outside the decision procedure entirely.

There is a limit here that deserves stating plainly, because it bounds what any of this can claim. Schneider's analysis of enforceable security policies establishes that execution monitors — mechanisms that observe a system's execution and halt it on a bad step — can enforce exactly the class of <span class="ix" data-ix="safety property">safety properties</span>, those whose violation is witnessed by some finite prefix of an execution [@schneider-enforceable]. Properties that are not of this form cannot be enforced by a monitor at all. Two consequences matter for governance.
...
**Listing 7.2 — The same intent expressed as Nornyx policy rules.** Patterned on the real rule vocabulary of `examples/governed_delivery_control_plane.nyx` and `nornyx/examples/org_policies.nyx`, with the rule tokens adapted to Atlas. Compare it with Listing 7.1: the structure is far more compact, and the compression is not free.
...
Two observations generalize beyond these three. First, languages that fix the request structure — Cedar's principal/action/resource/context, XACML's attribute categories — pay a real cost in flexibility and buy a real benefit in reasoning: a fixed structure is what makes it possible to answer questions *about* the policy rather than only questions *for* it. Second, a language that leaves the decision domain to the caller, as Rego does by design, is not thereby unsuitable; it means the decision domain is defined by the surrounding system's conventions, and a governance deployment must supply that definition explicitly rather than inherit it. Chapter 26 returns to all three when discussing how a governance contract can be projected onto an external policy engine acting as an independent enforcement point.
