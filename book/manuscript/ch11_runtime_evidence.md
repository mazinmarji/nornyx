...
Table 11.1 sets the three side by side on the dimensions that matter.
...
None of this makes logs and telemetry second-class; they answer questions evidence cannot. The discipline is to stop asking them to answer questions they were never designed for, and to add a third stream whose obligations are explicit.
...
Producers come in recognizable kinds. A *framework adapter* runs inside the agent's own process, observing the call it wraps and recording the decision. A *synthetic harness* produces records in a test or demonstration setting, where the surrounding behavior is fabricated on purpose. An *external runtime* — a gateway, a sandbox, a service-mesh sidecar — produces records from outside the agent's process. They differ in the respect that dominates all others: whether the producer is in a position to be wrong, or to lie, about what it records.
...
**Listing 11.1 — Validating a supplied evidence stream against a specific contract revision.** From `docs/agentic-network/06_RUNTIME_EVIDENCE.md` in the repository. Every argument is a binding: the contract fixes the declarations, the lock fixes the artifacts and the evidence schema version, the evaluation instant fixes which memberships and approvals were effective. Validation reads local files only — no listener, collector, or daemon is involved.

The output should state its own limits, because the natural reading of "validation passed" is "the system behaved correctly," and that reading is wrong.
...
One design response is to make the limit travel with the output. Listing 11.2 is that idea in its most literal form.
...
**Listing 11.2 — A proof boundary compiled into the validator.** From `nornyx/agentic_evidence.py` in the repository. These sentences are emitted inside every validation report, so a reader who has only the report still has the boundary. The lesson generalizes: a limitation stated only in documentation is separated from the artifact within one forwarding step.

> **Nornyx in practice.** As implemented at the snapshot, reports carry the sentences in Listing 11.2 alongside a `safety` block asserting that validation itself called no models, executed no tools, used no external connectors, used no network, and executed no producers. The residual-risk list in the repository's security-boundary documentation is equally direct: "Evidence is supplied, not observed: omission and fabrication are outside Nornyx's proof surface" (`docs/agentic-network/08_SECURITY_BOUNDARIES.md`).
...
A fifth element is not content but text: the stated proof boundary, carried inside the report rather than in a separate document that travels badly. A package lacking the validator version and the stated limits will be over-read by every reader who did not build it.

Packages also need a containment rule: referenced artifacts must resolve inside the package directory, and escapes must be errors rather than warnings. Without it the package is not a bounded object, because it points at files that may move, may be symbolic links elsewhere, and may not exist on the auditor's machine.

> **Nornyx in practice.** As implemented at the snapshot, per-event artifact references take the form `evidence_artifact: {path, sha256}`; the path resolves relative to the events file's own directory and must stay inside it, with escapes, symbolic links, and missing files failing closed (`AN_EVT_ARTIFACT_MISSING`) and content mismatches failing closed (`AN_EVT_ARTIFACT_HASH_MISMATCH`) (`nornyx/agentic_evidence.py`). Reports are deterministic and carry the schema identifier and version validated against, the four binding digests, event and mission counts, sorted diagnostics, and the limitation sentences of Listing 11.2. Assembling these into an auditor-facing bundle is a pipeline step rather than a command: the reference continuous-integration script performs "audit-package assembly" as one of its fourteen steps (`scripts/agentic_network_ci.py`).
...
<figure class="nx-fig" id="fig-11-2">
  ...
  <figcaption><b>Figure 11.2 — The evidence lifecycle.</b> Evidence is designed at authoring time and merely emitted at runtime. The digests that runtime records carry are fixed during generation, which is why the arrow from generation to runtime is load-bearing: if generation is not deterministic, every downstream binding becomes ambiguous. The teaching purpose is to place retention, privacy, and import (Sections 11.6 and 11.7) as concerns of the last two stages rather than as afterthoughts.</figcaption>
</figure>
...
> **Nornyx in practice.** As implemented at the snapshot, the external-evidence surface is deliberately narrow: exactly two importers exist, `syft` for software bills of materials and `gitleaks` for secret scans (`nornyx/cli.py`; `ADAPTER_PARSERS` in `nornyx/package_scanner.py`), and any other tool name exits with `UNSUPPORTED_EVIDENCE_TOOL`. The import path never runs the tool. When an adapter is declared but no report path is supplied, the recorded status is `unavailable` with the detail "external tools are not executed automatically; provide report_path to import evidence"; if the tool's binary is on the executing machine's path, the detail appends "command is available but not executed by Nornyx" (`nornyx/package_scanner.py`). Every normalized record carries a `source` of `built_in_scanner` or `external_adapter` and a `status` of `observed` or `imported`, so provenance survives normalization — the Section 11.4 requirement, implemented as two fields. A required adapter whose report is absent produces an error diagnostic and an overall failing status rather than a silent skip.
...
1. A team proposes to satisfy its evidence requirement by raising its log level to `DEBUG` and extending retention to one year. Using Table 11.1, identify the two dimensions on which this still fails, and why neither is fixed by verbosity.
2. Distinguish supplied from observed evidence. For an in-process framework adapter, name the two failure modes a conforming, fully validated stream cannot rule out, and explain why each is invisible from inside the stream.
3. An evidence package contains records, three referenced artifacts with digests, and a report stating `status: pass`. The validator version is not recorded. Describe a specific way a reader two years later could be misled, and the one field that prevents it.
...
1. Write an evidence contract, in the form of Table 11.2, for Northstar's Forge agent opening a pull request against `northstar/payments-api`. Cover the event types, binding fields, ordering constraints, validation procedure, and — in two separate rows — what a passing validation establishes and what it does not. Then name the producer for each event type and state its coverage boundary.
2. Using the repository at the book's snapshot, run `nornyx package evidence import` with an unsupported tool name and record the diagnostic code. Then declare an external evidence adapter without supplying a report path, run a package scan, and read the adapter execution report. Explain, from the observed `status` and `detail` values, what the report asserts about whether the external tool ran.
