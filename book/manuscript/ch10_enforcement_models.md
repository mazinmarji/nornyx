...
> - State why a correct decision does not constrain an action whose path to its effect never traverses an element that applies the decision.
...
- Enforcement is a topology property: a decision constrains only the paths that traverse the element applying it.
...
6. A platform team asserts that because every tool call is evaluated by the decision service, "the agent cannot act outside policy." Name the topology assumption hidden in that sentence, and describe one concrete enforcement gap the assumption conceals.
