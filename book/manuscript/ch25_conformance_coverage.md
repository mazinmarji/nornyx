...
Read that as a claim-management rule rather than a dependency-management one. The `wrapped` label is not an abstract statement about CrewAI or LangGraph; it is a statement about *the versions whose dispatch behavior the test suite has actually exercised*. Extending it to an untested version would be the tier inflation Chapter 13 warned against, arriving through a dependency specifier rather than through prose.
...
Package metadata is evidence, and reading it precisely is a skill this section exercises on one small distribution.
