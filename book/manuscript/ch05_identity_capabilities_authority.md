...
> **Case study — Atlas.** Northstar declares Atlas as a governance identity in namespace `northstar.research` with subject `atlas`, bound to the CrewAI tool wrapper the Research & Insights pilot already uses. Its capability set is exactly three entries: `research.search_approved`, `research.summarize`, and `research.file_internal`. All three are low risk and none carries a gate — a deliberate statement, since Atlas's charter is meant to run without human intervention, and the way to keep it that way is to keep risky things *out* of the capability set rather than gate them inside it. `publish_external` and `purchase.*` are not gated capabilities of Atlas; they are not capabilities of Atlas at all. The distinction becomes operational in Chapter 7 and consequential in Chapter 10, where the publish request from Chapter 1's scenario meets an enforcement point.
...
A fifth component matters only once more than one agent exists: whether the capability may be **delegated**, and how deep. Section 5.6 returns to it.

Notice what a capability defined this way is *not*. It is not a token presentable as proof of authority; it is a declaration that an authority relation may exist. It is not a command, a script, or a credential. Keeping these out is deliberate: a declaration containing an endpoint and a key is a deployment artifact with a governance flavor, and cannot be reviewed, versioned, or published on the same terms.
...
The bug follows immediately from mixing them. An engineer implements "the coordinator hands the case to the escalation agent" and, finding the escalation agent cannot perform the next step, adds the missing capability at the point of handoff. The system now works — and now contains a rule that says, in effect, *receiving work confers the authority needed to do it*, an escalation primitive available to anyone who can cause a handoff. The correct behavior, summarized in Table 5.2, is that a handoff whose receiver lacks a required capability *fails*; the fix is to grant the receiver that capability deliberately, as a reviewable change, or to delegate it explicitly with bounds.
...
> **Assurance boundary.** Everything in this section is a *design-time* property of declared relations. That a delegation chain is well-formed, attenuated, and bounded is checkable from artifacts alone, and that check is deterministic. It is not a claim that the running system honored the chain: whether a delegate confined itself to the delegated scope depends on the enforcement point (Chapter 10), the evidence it produced (Chapter 11), and the assurance tier the deployment supports (Chapter 13). Validating a delegation graph proves the graph is sound, not that reality matched it.
...
- Status, validity intervals, and revocation records must survive the identity's retirement, so past decisions remain interpretable.
- A capability without enumerated actions and explicit scope is a category, and categories cannot be evaluated.
- Authority is computed per situation from identity, permission, scope, obligations, and time; storing it invites reuse outside that situation.
