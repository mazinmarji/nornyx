...
The transparency row explains a decision that otherwise looks like a gap. The CrewAI adapter declares agent invocation, task invocation, delegation, and handoff unsupported, and the declared reason is not "we ran out of time" but that these have no verified, stable public hook distinct from tool-level interception, so wrapping them "would depend on undocumented CrewAI internals" **[implemented]**. Narrower coverage over internals-dependent coverage is a claim that stays true across a framework patch release, chosen over one that covers more today and may quietly stop tomorrow.
