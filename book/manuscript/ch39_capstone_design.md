...
Thread E's requirement — a lower layer may narrow but never silently widen a superior control — becomes concrete as a repository layout for the <span class="ix" data-ix="governance source!workspace layout">governance source</span>. The `northstar-governance` repository holds two files that matter: a <span class="ix" data-ix="canonical policy">canonical policy</span> contract and a <span class="ix" data-ix="workspace manifest!capstone">workspace manifest</span>. Listing 39.1 shows both, exactly as built and verified for this capstone.
...
A capstone-scale system needs its authority nouns in one <span class="ix" data-ix="inventory!identity, capability, zone">consolidated inventory</span>, because the risks worth designing against are cross-application: an identity that quietly accumulates capabilities across contracts, a zone name that means different things in two repos, a capability string that one team treats as low-risk and another gates. Table 39.1 consolidates the inventory across all three applications.
...
The chapter's central deliverable is Table 39.2, the <span class="ix" data-ix="tier selection!capstone decision table">tier decision table</span>: every governed action class walked through Chapter 13's two questions — what does one failure cost, and who is trying to cause it — with the resulting tier and, critically, the named component that enforces it. A tier without an enforcing component is a wish.
