...
| `GRAPH_EDGE_WITHOUT_RELATION` / `INVALID_GRAPH_RELATION_PAIR` | An edge has no relation verb, or uses a known relation between disallowed source and target kinds | error, exit 1 | `nornyx/checker.py:144-168` |
| `UNKNOWN_GRAPH_RELATION` / `GRAPH_EVIDENCE_NODE_WITHOUT_REF` | The verb is outside the twenty-three typed relations; or an evidence node carries no `ref` | warning | `graph.edges`, `graph.nodes` |
