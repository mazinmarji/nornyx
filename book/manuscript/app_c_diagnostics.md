...
| `UNKNOWN_GRAPH_NODE_KIND` / `GRAPH_NODE_WITHOUT_REF` / `GRAPH_CYCLE` | A node kind is outside the core vocabulary and the active profile's `graph.node_kinds`; an `artifact`, `module` or profile-declared node has no `ref`; or the folded edge set forms a cycle (ADR-0046) | warning; `--strict` promotes | `graph.nodes`, `graph.edges` |
