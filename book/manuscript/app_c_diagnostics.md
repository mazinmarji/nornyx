...
on message text breaks on the next wording change; one that keys on codes does not. This appendix
catalogues the code families present in Nornyx at the audited revision `70d2b40ad792`.

**Coverage statement.** This is a *representative* catalogue, not an exhaustive one — the
agentic-network static checks alone contribute more than one hundred and fifty codes. Every code
printed below was extracted from the repository by direct search or observed in a live command run;
where a family is sampled rather than complete, the section says so and names the file to search.
No code here is invented.
...
`MISSING_<SINGULAR>_NAME` from the singular of a list block (`MISSING_POLICY_NAME` and its
siblings); `MISSING_GOAL_<FIELD>` and `INVALID_GOAL_<FIELD>` from a goal field name; and the
delegation and handoff gate codes, composed from a prefix constant to give
`AN_DELEGATION_GATE_REQUIRED`, `AN_HANDOFF_APPROVAL_REQUIRED`, `AN_HANDOFF_EGRESS_GATE_MISSING`,
and their siblings.
...
Raised by `nornyx check` and any command that validates a document.
...
| `MISSING_GRAPH_NODE_ID` / `_KIND`, `DUPLICATE_GRAPH_NODE_ID`, `DUPLICATE_GRAPH_EDGE`, `GRAPH_SELF_EDGE` | A node omits `id` or `kind`, an id or edge repeats, or an edge is a self-loop | error, exit 1 | `graph.nodes`, `graph.edges` |
...
| `GRAPH_EDGE_WITHOUT_RELATION` / `INVALID_GRAPH_RELATION_PAIR` | An edge has no relation verb, or uses a known relation between disallowed source and target kinds (core rules plus the active profile's `relationship_constraints`) | error, exit 1 | `GRAPH_RELATION_RULES` in `nornyx/checker.py` |
| `UNKNOWN_GRAPH_RELATION` / `GRAPH_EVIDENCE_NODE_WITHOUT_REF` | The verb is outside the twenty-three core relations and any the active profile declares; or an evidence node carries no `ref` | warning | `graph.edges`, `graph.nodes` |
| `UNKNOWN_GRAPH_NODE_KIND` / `GRAPH_NODE_WITHOUT_REF` / `GRAPH_CYCLE` | A node kind is outside the core vocabulary and the active profile's `graph.node_kinds`; an `artifact`, `module` or profile-declared node has no `ref`; or the `depends_on` edges form a cycle (ADR-0046) | warning; `--strict` promotes | `graph.nodes`, `graph.edges` |
| `GRAPH_VOCABULARY_PROFILE_UNRESOLVED` / `GRAPH_VOCABULARY_UNAVAILABLE` | `project.profile` names no built-in profile so the graph was checked against the core vocabulary only; or the profile registry could not be consulted at all, so the graph is not accepted on core semantics (ADR-0046) | warning / error, exit 1 | `project.profile` |
| `INVALID_CONTRACTS_BLOCK` / `INVALID_CONTRACT_ENTRY` / `MISSING_CONTRACT_NAME`, `UNKNOWN_CONTRACT_GRAPH_REFERENCE` / `_APPROVAL_REFERENCE` / `_BUDGET_REFERENCE` | The contracts block or an entry is malformed, or references an undeclared node, approval, or budget | error, exit 1 | `contracts` |
...
| `APPROVAL_NON_HUMAN` / `APPROVAL_NON_HUMAN_AUTHORITY` | An assertion's claimed actor type is not `human` or is denied; or `accountable_authority` is non-human | **deny decision**; error | `nornyx/agentic/authz.py:1040-1042`; `approvals.py:494-499` |
| `APPROVAL_ROLE_INVALID` / `APPROVAL_REVISION_MISMATCH` | The approver's role is outside eligible ∪ required roles; or the assertion's revision differs from the contract's or the declared binding | deny decision | `authz.py:1030-1045` |
...
| `EXCEPTION_EXPIRED` / `_INTERVAL_INVALID` / `_TIME_INVALID`, `EXCEPTION_RENEWAL_NOT_ALLOWED` / `_APPROVAL_MISSING` / `_FORK_INVALID` / `_INTERVAL_INVALID` / `_REFERENCE_INVALID` | The waiver's window has passed or is malformed, or renewal violates the declared renewal policy | error, exit 1 | The exception record; `renewal_policy` |
...
| `CHANGE_NON_HUMAN_APPROVER` / `_AUTHORITY`, `CHANGE_APPROVER_ROLE_UNAUTHORIZED`, `CHANGE_SCOPE_HASH_MISMATCH` | Approved by a non-human or unauthorised role; or the declared scope no longer hashes to its recorded value | error, exit 1 | The change record; `change_scope_hash` |
...
| `EVIDENCE_ARTIFACT_UNAVAILABLE` / `_ROOT_REQUIRED`, `EVIDENCE_ARTIFACT_HASH_MISMATCH`, `EVIDENCE_HASH_SUBSTITUTION` | The artifact cannot be read, lies outside the permitted root, does not hash to the declared value, or its hash appears substituted rather than recomputed | error; exit 2 for path failures | The record's `artifact`, `content_hash` |
| `EVIDENCE_REVISION_MISMATCH` | A record's subject revision differs from the set's | error | `subject_revision` |
| `EVIDENCE_STALE` / `_GENERATED_IN_FUTURE` / `_TIME_INVALID` | Past its freshness window, dated in the future, or malformed | error, exit 1 | `generated_at`, `expires_at`, the `--as-of` supplied |
| `EVIDENCE_DEPENDENCY_UNSATISFIED` / `_INVALID` / `_CYCLE`, `EVIDENCE_DUPLICATE_ID` / `_ID_INVALID` / `_SOURCE_VALUE_INVALID`, `EVIDENCE_IMPORT_ERROR` | Dependencies unmet, malformed, or circular; repeated or malformed identity or source; a failed import | error, exit 1 | The evidence set |
| `ARCH_EVIDENCE_TOOL_MISMATCH` / `_SCHEMA_MISMATCH` / `_CHECK_UNKNOWN` / `_STATUS_INCONSISTENT`, `ARCH_EVIDENCE_STALE` / `_GENERATED_IN_FUTURE` / `_TIME_INVALID` / `_TIME_REQUIRED` | A report names the wrong tool, schema, or an undeclared check, reports inconsistently, or fails freshness | error | `schemas/architecture_report_v1.schema.json` |
...
|---|---|---|
| Identity invariants | `AN_NON_HUMAN_APPROVAL_INVALID`, `AN_IDENTITY_DUPLICATE`, `AN_IDENTITY_ROLE_UNKNOWN`, `AN_IDENTITY_BINDING_DUPLICATE` | An identity claims human authority or approval rights, or repeats an id, subject, or framework binding |
| Reference integrity | `AN_CAPABILITY_UNKNOWN`, `AN_TRUST_ZONE_UNKNOWN`, `AN_MEMBERSHIP_UNKNOWN`, `AN_GATE_UNKNOWN`, `AN_EVIDENCE_UNKNOWN`, `AN_APPROVAL_UNKNOWN` | A reference does not resolve to a declared record |
...
| Approval declaration | `AN_APPROVAL_DECLARATION_MISSING`, `AN_APPROVAL_ACTION_MISSING`, `AN_APPROVAL_DECLARED_ROLE_UNAUTHORIZED`, `AN_APPROVAL_MODULE_ROLE_OMITTED`, `AN_APPROVAL_ACCOUNTABLE_AUTHORITY_MISMATCH` | The network authority approval is absent, misses a required action, or contradicts the module's roles |
| Approval record | `AN_APPROVAL_HUMAN_REQUIRED`, `AN_APPROVAL_ROLE_INVALID`, `AN_APPROVAL_PRODUCER_OUTSIDE_MODULE_AUTHORITY`, `AN_APPROVAL_RECORD_MISSING`, `AN_APPROVAL_RECORD_INVALID`, `AN_APPROVAL_REVOKED`, `AN_APPROVAL_EXPIRED`, `AN_APPROVAL_EXPIRY_EXCESSIVE` | The record is absent, non-human, wrongly-roled, revoked, expired, or excessively long-lived |
...
| Delegation | `AN_SELF_DELEGATION`, `AN_DELEGATION_ACTION_ESCALATION`, `AN_DELEGATION_SCOPE_ESCALATION`, `AN_DELEGATION_DEPTH_EXCEEDED`, `AN_ONWARD_DELEGATION_DENIED`, `AN_DELEGATION_CHAIN_CYCLE`, `AN_DELEGATOR_MEMBERSHIP_REQUIRED`, `AN_DELEGATION_INTERVAL_EXCEEDS_PARENT` | A delegation widens authority, exceeds depth, cycles, outlives its parent, or lacks the membership grounding it |
| Handoff | `AN_HANDOFF_AUTHORITY_ESCALATION`, `AN_HANDOFF_SELF`, `AN_HANDOFF_MEMBERSHIP_REQUIRED`, `AN_HANDOFF_MISSION_UNKNOWN`, `AN_HANDOFF_STATUS_CONTRADICTION` | A handoff transfers more authority than the source holds, or is internally contradictory |
...
| Protocol targets | `AN_PROTOCOL_TRANSITION_NOT_ALLOWED`, `AN_PROTOCOL_EGRESS_GATE_MISSING`, `AN_PROTOCOL_INGRESS_GATE_MISSING`, `AN_PROTOCOL_APPROVAL_REQUIRED`, `AN_PROTOCOL_IDENTITY_UNAUTHORIZED`, `AN_PROTOCOL_CAPABILITY_UNAUTHORIZED` | A declared MCP or A2A target is ungated, unapproved, or advertised by an identity that does not hold the capability in that zone |
| Sensitive sharing | `AN_SENSITIVE_SHARE_BOUNDARY_MISSING`, `AN_DELEGATION_SENSITIVE_SHARING`, `AN_HANDOFF_SENSITIVE_SHARING`, `AN_SHARE_NOT_ALLOWED_SOURCE`, `AN_SHARE_NOT_ALLOWED_TARGET`, `AN_SHARE_CATEGORY_UNKNOWN` | A record shares `secrets`, `credentials`, `tokens`, or `private_memory` across a prohibited boundary |
| Relations | `AN_RELATION_ENDPOINT_KIND_INVALID`, `AN_RELATION_CONTRADICTORY`, `AN_RELATION_APPROVER_NOT_HUMAN`, `AN_RELATION_UNDECLARED_AUTHORITY` | A relation is wrongly typed, contradicts what it references, or asserts undeclared authority |
...
| `AN_LOCK_NETWORK_MISMATCH` / `_REVISION_MISMATCH`, `AN_LOCK_PROFILE_MISMATCH` / `_MODULE_MISMATCH` | Network id, subject revision, or a resolved pack's id, version, or content hash differs | exit 1 | Lock, contract, `profiles resolve` |
| `AN_LOCK_SCHEMA_MISMATCH` / `_CHECKS_MISMATCH` / `_PROTOCOL_MISMATCH`, `AN_LOCK_APPROVAL_MISMATCH` / `_EVIDENCE_MISMATCH` | Block schemas, the runtime-events schema version, structural checks, protocol declarations, or composed approval and evidence references differ | exit 1 | The composition |
...
`AN_EVT_SCHEMA_INVALID`; `AN_EVT_SCHEMA_LOCK_MISMATCH` (the envelope's schema version differs from
the lock's — a 1.0 stream is never silently upgraded); `AN_EVT_LOCK_STALE` (the supplied lock no
longer verifies); `AN_EVT_NETWORK_MISMATCH`, `AN_EVT_REVISION_MISMATCH`,
`AN_EVT_CONTRACT_MISMATCH`, `AN_EVT_LOCK_MISMATCH` (a per-event binding field differs);
`AN_EVT_FIELD_REQUIRED` (a field the event type requires is absent);
`AN_EVT_DECISION_CONTRADICTION`; `AN_EVT_INVALID`.
...
`AN_EVT_HANDOFF_UNKNOWN`, `AN_EVT_ZONE_UNKNOWN`, `AN_EVT_APPROVAL_UNKNOWN`;
`AN_EVT_CAPABILITY_NOT_HELD` (an allowance or tool use unbacked by a held or delegated capability);
`AN_EVT_DELEGATION_ACTOR_MISMATCH` / `_EXPIRED` / `_REVOKED`; `AN_EVT_HANDOFF_PARTY_MISMATCH`;
`AN_EVT_APPROVAL_NON_HUMAN` (an `approval_granted` event names a non-human approver — grants only,
so a *refused* non-human approval still validates); `AN_EVT_APPROVAL_ROLE_INVALID`.
...
(timestamps decrease); `AN_EVT_DEPENDENCY_MISSING` (a `depends_on` target absent or at a higher
sequence); and the paired-transition codes `AN_EVT_TOOL_WITHOUT_ALLOWANCE`,
...
**Replay.** `AN_EVT_REPLAY` fires when two events share a content fingerprint — a digest of the
event with transport fields removed: `event_id` and `sequence` in the legacy modes, plus
`timestamp` in explicit mode, so a restamped duplicate is still caught. Occurrence identity is part
of the fingerprint, so identical work in a new occurrence or attempt is correctly not replay.

**Sensitive sharing and zone crossing.** `AN_EVT_CROSSING_NOT_DECLARED` (not an allowed transition
of the source zone); `AN_EVT_CROSSING_UNGOVERNED` (no governing gate);
`AN_EVT_CROSSING_APPROVAL_MISSING`; `AN_EVT_SENSITIVE_SHARING` (`secrets`, `credentials`,
`tokens`, or `private_memory` crosses a prohibited boundary); `AN_EVT_SHARE_NOT_ALLOWED` (the
category is outside the zone's allowlist).
...
(prefix `AN_ADAPTER_`): `APPROVAL_NON_HUMAN`, `APPROVAL_NOT_GRANTED`, `APPROVAL_ROLE_INVALID`,
`CAPABILITY_DENIED`, `CAPABILITY_UNKNOWN`, `CONTRACT_INVALID`, `CROSSING_APPROVAL_REQUIRED`,
`DELEGATION_INACTIVE`, `DELEGATION_UNKNOWN`, `EVIDENCE_INVALID`, `FRAMEWORK_MISMATCH`,
`HANDOFF_AUTHORITY`, `HANDOFF_UNKNOWN`, `HOOK_MISSING`, `IDENTITY_UNKNOWN`, `LOCK_INVALID`,
`LOCK_STALE`, `PROFILE_MISSING`, `REQUEST_MALFORMED`, `SENSITIVE_SHARING`, `SHARE_NOT_ALLOWED`,
`ZONE_CROSSING_DENIED`. Compatibility codes only, not public interface guarantees.

## C.10 Governed-package codes

| Code | Raised when | Fail-closed behaviour |
|---|---|---|
| `INVALID_GOVERNED_PACKAGE` / `_PROFILE` / `_MISSION` / `_CHANGE` / `_RISK_TIER`, `INVALID_INSTALLATION_POLICY`, `INVALID_SAFETY_BOUNDARY` | The declaration or one of its parts is malformed, or a permissive installation or safety flag is set | validation fails, exit 1 |
...
| `PACKAGE_WITHOUT_README` / `_WITHOUT_LICENSE` / `_CONTAINS_BINARY_FILES` / `_CONTAINS_MINIFIED_FILES` / `_HAS_UNCLEAR_REMOTE_ENDPOINTS` | Scanner observations about the artifact set | risk-surface findings, not automatic failures |
