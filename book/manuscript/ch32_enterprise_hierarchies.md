...
## 32.2 The fourth analogy: home and visited networks

This book's recurring analogy has one property left to lend, and it is the one enterprise architects most often need. When a mobile subscriber roams, two administrative domains apply policy to the same session at the same time [@3gpp-ims; @gsma-volte]. The <span class="ix" data-ix="home network policy">home network</span> holds the subscriber profile: which services the subscription includes, which are barred, what the subscriber is entitled to. The <span class="ix" data-ix="visited network policy">visited network</span> holds local policy: what its own bearer resources permit, what local regulation requires, what its own agreements allow. Service control is anchored in the home network's session control function even when the subscriber is attached abroad, so the entitlements travel with the subscription rather than with the location.

The structural lesson transfers cleanly. The visited network can *restrict* — it can refuse a service its own resources or regulations do not permit — and it cannot *grant* a service the home subscription does not contain. Attachment to a permissive domain does not create entitlement. That is precisely the narrowing rule of Section 32.1, expressed by an industry that had to make it work across thousands of bilateral relationships and could not rely on any party's good intentions.

Two further details are worth borrowing. First, the layering is explicit about *which domain decides what*: the reference points between the domains are named, and a query that crosses one has a defined shape. An enterprise policy stack that cannot say which layer decided a given element has not achieved this. Second, the subscriber profile is retrieved from one authoritative store rather than replicated into each visited network. Replication is exactly what the opening scenario's fourteen drifted copies were.

Where the analogy stops matters as much as where it holds, and there are four boundaries.

Roaming policy is enforced in the bearer path by infrastructure the handset cannot decline. Enterprise governance layering, as Chapter 31 laboured, is usually cooperative. The analogy describes the *shape* of layered authority; it does not transfer the enforcement guarantee.

Roaming interworking rests on standardized interfaces and bilateral commercial agreements negotiated in advance. Enterprise policy layers have no standards body, no conformance regime, and frequently no written agreement between the layers at all — which is why so much of this chapter is about making disagreement *visible* rather than about resolving it automatically.

Roaming has a single authoritative subscriber store per home network. An enterprise typically has several plausible policy sources, and deciding which is canonical is a political act before it is a technical one.

And the determinism boundary from Chapter 2 applies unchanged. Roaming policy is evaluated over a closed set of subscription attributes; enterprise agentic policy is evaluated over declared concepts about a system whose proposals come from a probabilistic planner. The layering transfers; the closed input space does not.

## 32.3 What the repository implements today

Three implemented mechanisms cover part of the stack. It is worth being exact about which part, because the gap is the subject of Section 32.5.
...
3. Using the roaming analogy, explain why "the agent is running in a permissive environment" is not an argument that it holds an authority. Then state two properties of roaming that do not transfer.
...
- [@3gpp-ims] — the reference architecture behind this chapter's analogy; read the roaming and service-control sections for how home and visited domains divide authority.
- [@gsma-volte] — the operator-facing view of the same layering, useful for seeing how bilateral agreements sit underneath the technical interfaces.
...
- [@nornyx-repo] — the workspace module, the policy reference mechanism, the composition provenance records, the exception schema, and the multi-repository case study whose three defects Section 32.3 recounts.
