...
Now the semantics, stated exactly. <span class="ix" data-ix="deny rule">Deny rules</span> are matched at planning time by a function that looks for risk-category keywords **in the rule text** and, when one is found, for a corresponding set of keywords **in the flow step's text** **[implemented]**. The step's text is the concatenation of its kind, its reference, its action, and all its string values, lower-cased. Table 17.1 is the complete matching model — there is nothing else.

| If the rule text contains… | …the step is blocked when its text contains |
|---|---|
| `production` | `production`, `prod`, `deploy`, or `release` |
| `secret` | `secret`, `token`, or `credential` |
| `destructive` | `delete`, `destroy`, `drop`, `wipe`, `reset`, or `remove` |
| `connector` | (the step is of kind `connector`) |
| `self_modification` or `self-modification` | `self_modification`, `self-modification`, or `modify self` |

**Table 17.1 — The complete deny-matching model.** Five risk categories, substring-based, applied only to flow steps of kind `agent`. The teaching purpose is calibration: this is a small, deterministic, reviewable computation, and a rule token outside these five families — `deny purchase_anything`, say — matches nothing at all and therefore blocks nothing, silently. Knowing the table is the difference between reading a deny list as a specification of intent and reading it as a description of what the tool will do.
...
The reference form is `<path>#<PolicyName>`, and the path may be a local `.nyx` contract or a workspace manifest; the pair above is Northstar's, patterned on the bundled `nornyx/examples/org_policies.nyx` and `nornyx/examples/governed_service.nyx`.
...
Listing 17.5 shows the diagnostic stream from a contract with three deliberate defects.
...
We can now build the index card from the opening scenario into a real document. The full contract runs to about a hundred lines; Listing 17.6 shows the parts that carry the governance, and the whole file was verified by running the tool.
...
**Listing 17.7 — A real denial, and an honest reading of it.** Real output from `nornyx policy-check` on the Atlas contract with a publishing step added; the run's summary changes to `"blocked": 1`. The denial fired because the rule token contains `production` and the step text contains `release` — a *risk-category* match, not a match on the rule's full name. Nothing in the toolchain understands "publishing a brief"; it recognizes a step that smells like a production-class action under a policy that forbids production-class actions without approval.
